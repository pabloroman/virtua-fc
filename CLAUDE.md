...
- **Control plane** (`pgsql_control` connection): cross-tenant data — identity, leaderboards, reference data, game directory. Read-heavy, small. `User`, `ManagerStats`, `ManagerTrophy`, `TournamentSummary`, `Team`, `Competition`, `GamePlayerTemplate` (real-world roster source, source of player biography) and its audits, onboarding tables.

**Rules:**
- New cross-tenant models declare `protected $connection = 'pgsql_control'`.
- New per-game models stay on the default connection.
- **Never JOIN across planes.** Even though both connections currently resolve to the same physical DB, JOINs that cross the boundary will break the moment they're split. Use a service-layer call that issues separate queries on each connection — pluck the small set from one plane, then `whereIn` against the other. For hot paths where two-step queries regress performance, denormalize the needed attribute onto the consuming plane (see the `is_reserve_squad` and player-biography precedents).
...
- **No FK constraints across planes.** Tenant migrations can reference control table columns logically (e.g., `team_id`), but must not declare a database-level `foreign(...)->references(...)->on('teams')`. Postgres cannot enforce FKs across two physical instances, and the constraint becomes a hard error the day the planes split.
- **No Postgres-only SQL on tenant tables.** Tenant queries must work under both Postgres and SQLite drivers. Avoid `EXTRACT(YEAR FROM AGE(...))` (use `PlayerAge::dateOfBirthCutoff`), `::jsonb` casts, `jsonb_typeof` / `jsonb_exists_any`, and `ON CONFLICT` (use `upsert()` or split insert+ignore). Driver-portable Eloquent (`whereJsonContains`, `whereDate`, etc.) is the rule.
- Migrations target a specific plane. Control-plane schema goes in `database/migrations/control/`, tenant in `database/migrations/tenant/`. The flat `database/migrations/` dir holds historical migrations from before the split.

**Runtime guard:** `App\Database\CrossPlaneQueryGuard` (registered from `AppServiceProvider::boot`, gated by `DATABASE_PLANES_GUARD_ENABLED`) inspects every executed query and throws if the tables it touches sit on a different plane than the connection. Off by default; flip it on locally (`DATABASE_PLANES_GUARD_ENABLED=true`) when developing a feature that touches both planes to make sure new code stays single-plane. The historical OOM/timeout regressions that caused the guard to be off-by-default have been resolved — every previously-annotated `PLANES-SEAM` site has been re-split.
