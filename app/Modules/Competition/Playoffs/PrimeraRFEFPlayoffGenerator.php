...
class PrimeraRFEFPlayoffGenerator implements PlayoffGenerator
{
    ...
    {
        // Both bracket finals must be completed for the playoff to be over.
        $pendingFinals = CupTie::where('game_id', $game->id)
        ...
        // Make sure both bracket finals actually exist (not just "zero pending").
        $finalsCount = CupTie::where('game_id', $game->id)
            ->where('competition_id', $this->competitionId)
            ->where('round_number', $this->getTotalRounds())
            ->count();
