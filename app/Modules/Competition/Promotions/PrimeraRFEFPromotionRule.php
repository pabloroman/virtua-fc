...
class PrimeraRFEFPromotionRule implements SelfSwappingPromotionRule
{
    ...
    public function getPromotedTeams(Game $game): array
    {
        ...
        // Playoff winners: two bracket final winners from ESP3PO, or a
        // simulated fallback when no playoff matches were generated (happens
        // when the player isn't in either ESP3 group).
        $playoffWinners = $this->playoffWinners($game);

        if (empty($playoffWinners)) {
            $playoffWinners = $this->simulatedPlayoffStandIns($game, $promoted);
        }

        return array_merge($promoted, $playoffWinners);
    }
