        ...
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
                    ...
                    <div class="bg-accent-red/10 rounded-lg p-4">
                        <h3 class="text-sm font-semibold text-red-800 mb-3 flex items-center gap-1.5">
                            ...
                            @if(count($offseasonRecap['departures']) > 0)
                                <span class="text-xs font-normal text-accent-red">({{ count($offseasonRecap['departures']) }})</span>
                            @endif
                        </h3>
                        @if(empty($offseasonRecap['departures']))
                            <p class="text-sm text-accent-red/70 italic">{{ __('game.no_departures') }}</p>
                        @else
                    ...
                    <div class="bg-accent-green/10 rounded-lg p-4">
                        <h3 class="text-sm font-semibold text-green-800 mb-3 flex items-center gap-1.5">
                            ...
                            @if(count($offseasonRecap['arrivals']) > 0)
                                <span class="text-xs font-normal text-accent-green">({{ count($offseasonRecap['arrivals']) }})</span>
                            @endif
                        </h3>
                        @if(empty($offseasonRecap['arrivals']))
                            <p class="text-sm text-accent-green/70 italic">{{ __('game.no_arrivals') }}</p>
                        @else
                            <div class="space-y-1.5">
            ...
            <div class="bg-surface-800 rounded-xl shadow-xs border border-border-strong p-5 md:p-6 mb-6">
                    ...
                    <div class="flex items-start md:items-center justify-between gap-3">
                        <div class="flex-1">
                            <div class="flex items-center gap-2 mb-1.5">
                                ...
                                <h3 class="text-xs font-semibold text-accent-gold uppercase tracking-wide">{{ __('game.season_objective') }}</h3>
                            </div>
                            <div class="text-lg md:text-xl font-bold text-amber-950">{{ __($seasonGoalLabel ?? 'game.goal_top_half') }}</div>
                            <div class="text-xs text-accent-gold mt-0.5">{{ __('game.board_expects_position', ['position' => $seasonGoalTarget ?? 10]) }}</div>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex flex-col md:flex-row gap-3 mb-5">
                    ...
                    <div class="flex items-center gap-3 bg-surface-700/50 border border-border-strong rounded-lg px-4 py-3 flex-1">
                        <div class="w-8 h-8 rounded-lg bg-surface-600 flex items-center justify-center shrink-0">
            ...
            <div class="bg-surface-800 rounded-xl shadow-xs border border-border-strong p-5 md:p-6 mb-6">
                ...
                <div class="bg-accent-gold/10 border border-accent-gold/20 rounded-lg p-3">
                    <h4 class="text-xs font-semibold text-accent-gold uppercase tracking-wide mb-2 flex items-center gap-1.5">
                    ...
                    <ul class="space-y-1">
                        ...
                        <li class="text-sm text-accent-gold flex items-start gap-1.5">
