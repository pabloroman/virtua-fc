                            ...
                            <li class="col-span-1 flex flex-col rounded-lg bg-white text-center shadow">
                                <div class="flex flex-1 flex-col p-8 space-y-3">
                                    ...
                                    <dl class="flex flex-col justify-between">
                                        <dd class="text-sm text-gray-500">Temporada {{ $game->seasonName }}</dd>
                                        @if($game->currentDate)
                                            <dd class="mt-2 mb-2">
                                                <span class="inline-flex items-center rounded-full bg-green-50 px-2 py-1 text-xs font-medium text-green-700 ring-1 ring-inset ring-green-600/20">{{ $game->currentDate->format('d/m/Y') }}</span>
                                            </dd>
                                        @endif
                                    </dl>
                                    <div class="grid">
{{--                                        <x-primary-button class="place-self-center text-md !p-0">--}}
{{--                                            <a class="inline-flex px-4 py-2" href="{{ route('game.results', $game) }}">Seleccionar</a>--}}
{{--                                        </x-primary-button>--}}
                                    </div>
                                </div>
                            </li>
