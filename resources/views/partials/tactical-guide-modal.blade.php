...
<x-modal name="tactical-guide" max-width="3xl">
    <div class="p-5 space-y-6 max-h-[80vh] overflow-y-auto">
        ...
        <div class="flex items-center justify-between">
            <h2 class="text-lg font-bold text-slate-900">{{ __('game.tactical_guide_title') }}</h2>
            <button
                type="button"
        ...
        <div class="bg-slate-50 border border-slate-200 rounded-lg p-4">
            <p class="text-sm text-slate-600">{{ __('game.tactical_guide_intro') }}</p>
        </div>
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-slate-200 text-sm">
                        <thead class="bg-slate-50">
                            <tr>
                                <th class="px-4 py-2.5 text-left font-semibold text-slate-700">{{ __('game.tg_formation') }}</th>
                                <th class="px-4 py-2.5 text-center font-semibold text-slate-700">{{ __('game.tg_your_goals') }}</th>
                                <th class="px-4 py-2.5 text-center font-semibold text-slate-700">{{ __('game.tg_goals_conceded') }}</th>
                                <th class="px-4 py-2.5 text-left font-semibold text-slate-700 hidden md:table-cell">{{ __('game.tg_profile') }}</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-slate-100">
                            @foreach($guideFormations as $f)
                            <tr>
                                <td class="px-4 py-2.5 font-mono font-semibold text-slate-900">{{ $f['name'] }}</td>
            ...
            <div class="bg-white rounded-lg shadow overflow-hidden">
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-slate-200 text-sm">
                        <thead class="bg-slate-50">
                            <tr>
                                <th class="px-4 py-2.5 text-left font-semibold text-slate-700">{{ __('game.tg_mentality') }}</th>
                                <th class="px-4 py-2.5 text-center font-semibold text-slate-700">{{ __('game.tg_your_goals') }}</th>
                                <th class="px-4 py-2.5 text-center font-semibold text-slate-700">{{ __('game.tg_goals_conceded') }}</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-slate-100">
                            ...
                            <tr>
                                <td class="px-4 py-2.5 font-semibold text-slate-900">{{ __('game.mentality_' . $m['name']) }}</td>
                                <td class="px-4 py-2.5 text-center">
            ...
            <div class="bg-white rounded-lg shadow overflow-hidden">
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-slate-200 text-sm">
                        <thead class="bg-slate-50">
                            <tr>
                                <th class="px-4 py-2.5 text-left font-semibold text-slate-700">{{ __('game.tg_style') }}</th>
                                <th class="px-4 py-2.5 text-center font-semibold text-slate-700">{{ __('game.tg_your_goals') }}</th>
                                <th class="px-4 py-2.5 text-center font-semibold text-slate-700">{{ __('game.tg_goals_conceded') }}</th>
                                <th class="px-4 py-2.5 text-center font-semibold text-slate-700">{{ __('game.tg_energy') }}</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-slate-100">
                            @foreach($guidePlayingStyles as $s)
                            <tr>
                                <td class="px-4 py-2.5 font-semibold text-slate-900">{{ $s['label'] }}</td>
                                <td class="px-4 py-2.5 text-center">
            ...
            <div class="bg-white rounded-lg shadow overflow-hidden">
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-slate-200 text-sm">
                        <thead class="bg-slate-50">
                            <tr>
                                <th class="px-4 py-2.5 text-left font-semibold text-slate-700">{{ __('game.tg_pressing') }}</th>
                                <th class="px-4 py-2.5 text-center font-semibold text-slate-700">{{ __('game.tg_your_goals') }}</th>
                                <th class="px-4 py-2.5 text-center font-semibold text-slate-700">{{ __('game.tg_goals_conceded') }}</th>
                                <th class="px-4 py-2.5 text-center font-semibold text-slate-700">{{ __('game.tg_energy') }}</th>
                            </tr>
                        ...
                        <tbody class="divide-y divide-slate-100">
                            ...
                            <tr>
                                <td class="px-4 py-2.5 font-semibold text-slate-900">{{ $p['label'] }}</td>
                                <td class="px-4 py-2.5 text-center">
            ...
            <div class="bg-white rounded-lg shadow overflow-hidden">
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-slate-200 text-sm">
                        <thead class="bg-slate-50">
                            <tr>
                                <th class="px-4 py-2.5 text-left font-semibold text-slate-700">{{ __('game.tg_line') }}</th>
                                <th class="px-4 py-2.5 text-center font-semibold text-slate-700">{{ __('game.tg_your_goals') }}</th>
                                <th class="px-4 py-2.5 text-center font-semibold text-slate-700">{{ __('game.tg_goals_conceded') }}</th>
                                <th class="px-4 py-2.5 text-left font-semibold text-slate-700 hidden md:table-cell">{{ __('game.tg_note') }}</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-slate-100">
                            @foreach($guideDefensiveLines as $d)
                            <tr>
                                <td class="px-4 py-2.5 font-semibold text-slate-900">{{ $d['label'] }}</td>
                                <td class="px-4 py-2.5 text-center">
        ...
        <section>
            ...
            <div class="space-y-3">
                ...
                <div class="bg-slate-50 rounded-lg border border-slate-200 p-4 flex flex-col md:flex-row md:items-center md:justify-between gap-2">
                    <div>
                    ...
                    <span class="text-green-600 font-semibold text-sm shrink-0">+{{ round(($tacticalInteractions['counter_vs_attacking_high_line'] - 1) * 100) }}% {{ __('game.tg_your_goals') }}</span>
                </div>
                ...
                <div class="bg-slate-50 rounded-lg border border-slate-200 p-4 flex flex-col md:flex-row md:items-center md:justify-between gap-2">
                    ...
                    <span class="text-red-600 font-semibold text-sm shrink-0">{{ round(($tacticalInteractions['possession_disrupted_by_high_press'] - 1) * 100) }}% {{ __('game.tg_your_goals') }}</span>
                </div>
                ...
                <div class="bg-slate-50 rounded-lg border border-slate-200 p-4 flex flex-col md:flex-row md:items-center md:justify-between gap-2">
                    ...
                    <span class="text-green-600 font-semibold text-sm shrink-0">+{{ round(($tacticalInteractions['direct_bypasses_high_press'] - 1) * 100) }}% {{ __('game.tg_your_goals') }}</span>
                </div>
            </div>
        </section>
        ...
        <section>
            <div class="bg-slate-50 border border-slate-200 rounded-lg p-4">
                <h3 class="text-sm font-semibold text-slate-700 mb-2">{{ __('game.tg_legend') }}</h3>
