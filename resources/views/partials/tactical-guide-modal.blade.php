...
<x-modal name="tactical-guide" max-width="3xl">
    <div class="p-5 space-y-6 max-h-[80vh] overflow-y-auto">
        ...
        <div class="flex items-center justify-between">
            <h2 class="text-lg font-bold text-white">{{ __('game.tactical_guide_title') }}</h2>
            <button
                type="button"
                x-on:click="$dispatch('close-modal', 'tactical-guide')"
                class="p-1.5 rounded-lg text-slate-400 hover:text-slate-300 hover:bg-surface-700 transition-colors"
            >
        ...
        <div class="bg-surface-700/50 border border-white/10 rounded-lg p-4">
            <p class="text-sm text-slate-400">{{ __('game.tactical_guide_intro') }}</p>
        </div>
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-slate-200 text-sm">
                        <thead class="bg-surface-700/50">
                            <tr>
                                <th class="px-4 py-2.5 text-left font-semibold text-slate-300">{{ __('game.tg_formation') }}</th>
                                <th class="px-4 py-2.5 text-center font-semibold text-slate-300">{{ __('game.tg_your_goals') }}</th>
                                <th class="px-4 py-2.5 text-center font-semibold text-slate-300">{{ __('game.tg_goals_conceded') }}</th>
                                <th class="px-4 py-2.5 text-left font-semibold text-slate-300 hidden md:table-cell">{{ __('game.tg_profile') }}</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-slate-100">
                            @foreach($guideFormations as $f)
                            <tr>
                                <td class="px-4 py-2.5 font-mono font-semibold text-white">{{ $f['name'] }}</td>
                                <td class="px-4 py-2.5 text-center">
                                    <span class="{{ $f['attack'] > 1.0 ? 'text-accent-green' : ($f['attack'] < 1.0 ? 'text-accent-red' : 'text-slate-500') }}">
                                        {{ $f['attack'] == 1.0 ? '-' : ($f['attack'] > 1.0 ? '+' : '') . round(($f['attack'] - 1) * 100) . '%' }}
                                    </span>
                                </td>
                                <td class="px-4 py-2.5 text-center">
                                    <span class="{{ $f['defense'] < 1.0 ? 'text-accent-green' : ($f['defense'] > 1.0 ? 'text-accent-red' : 'text-slate-500') }}">
                                        {{ $f['defense'] == 1.0 ? '-' : ($f['defense'] > 1.0 ? '+' : '') . round(($f['defense'] - 1) * 100) . '%' }}
                                    </span>
                                </td>
                                <td class="px-4 py-2.5 hidden md:table-cell text-slate-500">
            ...
            <div class="bg-surface-800 rounded-lg shadow-sm overflow-hidden">
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-slate-200 text-sm">
                        <thead class="bg-surface-700/50">
                            <tr>
                                <th class="px-4 py-2.5 text-left font-semibold text-slate-300">{{ __('game.tg_mentality') }}</th>
                                <th class="px-4 py-2.5 text-center font-semibold text-slate-300">{{ __('game.tg_your_goals') }}</th>
                                <th class="px-4 py-2.5 text-center font-semibold text-slate-300">{{ __('game.tg_goals_conceded') }}</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-slate-100">
                            ...
                            <tr>
                                <td class="px-4 py-2.5 font-semibold text-white">{{ __('game.mentality_' . $m['name']) }}</td>
                                <td class="px-4 py-2.5 text-center">
                                    <span class="{{ $m['own_goals'] > 1.0 ? 'text-accent-green' : ($m['own_goals'] < 1.0 ? 'text-accent-red' : 'text-slate-500') }}">
                                        {{ $m['own_goals'] == 1.0 ? '-' : ($m['own_goals'] > 1.0 ? '+' : '') . round(($m['own_goals'] - 1) * 100) . '%' }}
                                    </span>
                                </td>
                                <td class="px-4 py-2.5 text-center">
                                    <span class="{{ $m['opponent_goals'] < 1.0 ? 'text-accent-green' : ($m['opponent_goals'] > 1.0 ? 'text-accent-red' : 'text-slate-500') }}">
            ...
            <div class="bg-surface-800 rounded-lg shadow-sm overflow-hidden">
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-slate-200 text-sm">
                        <thead class="bg-surface-700/50">
                            <tr>
                                <th class="px-4 py-2.5 text-left font-semibold text-slate-300">{{ __('game.tg_style') }}</th>
                                <th class="px-4 py-2.5 text-center font-semibold text-slate-300">{{ __('game.tg_your_goals') }}</th>
                                <th class="px-4 py-2.5 text-center font-semibold text-slate-300">{{ __('game.tg_goals_conceded') }}</th>
                                <th class="px-4 py-2.5 text-center font-semibold text-slate-300">{{ __('game.tg_energy') }}</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-slate-100">
                            @foreach($guidePlayingStyles as $s)
                            <tr>
                                <td class="px-4 py-2.5 font-semibold text-white">{{ $s['label'] }}</td>
                                <td class="px-4 py-2.5 text-center">
                                    <span class="{{ $s['own_xg'] > 1.0 ? 'text-accent-green' : ($s['own_xg'] < 1.0 ? 'text-accent-red' : 'text-slate-500') }}">
                                        {{ $s['own_xg'] == 1.0 ? '-' : ($s['own_xg'] > 1.0 ? '+' : '') . round(($s['own_xg'] - 1) * 100) . '%' }}
                                    </span>
                                </td>
                                <td class="px-4 py-2.5 text-center">
                                    <span class="{{ $s['opp_xg'] < 1.0 ? 'text-accent-green' : ($s['opp_xg'] > 1.0 ? 'text-accent-red' : 'text-slate-500') }}">
                                        {{ $s['opp_xg'] == 1.0 ? '-' : ($s['opp_xg'] > 1.0 ? '+' : '') . round(($s['opp_xg'] - 1) * 100) . '%' }}
                                    </span>
                                </td>
                                <td class="px-4 py-2.5 text-center">
                                    <span class="{{ $s['energy'] > 1.0 ? 'text-amber-600' : ($s['energy'] < 1.0 ? 'text-accent-green' : 'text-slate-500') }}">
            ...
            <div class="bg-surface-800 rounded-lg shadow-sm overflow-hidden">
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-slate-200 text-sm">
                        <thead class="bg-surface-700/50">
                            <tr>
                                <th class="px-4 py-2.5 text-left font-semibold text-slate-300">{{ __('game.tg_pressing') }}</th>
                                <th class="px-4 py-2.5 text-center font-semibold text-slate-300">{{ __('game.tg_your_goals') }}</th>
                                <th class="px-4 py-2.5 text-center font-semibold text-slate-300">{{ __('game.tg_goals_conceded') }}</th>
                                <th class="px-4 py-2.5 text-center font-semibold text-slate-300">{{ __('game.tg_energy') }}</th>
                            </tr>
                        ...
                        <tbody class="divide-y divide-slate-100">
                            ...
                            <tr>
                                <td class="px-4 py-2.5 font-semibold text-white">{{ $p['label'] }}</td>
                                <td class="px-4 py-2.5 text-center">
                                    <span class="{{ $p['own_xg'] > 1.0 ? 'text-accent-green' : ($p['own_xg'] < 1.0 ? 'text-accent-red' : 'text-slate-500') }}">
                                ...
                                <td class="px-4 py-2.5 text-center">
                                    ...
                                    @else
                                        <span class="{{ $p['opp_xg'] < 1.0 ? 'text-accent-green' : ($p['opp_xg'] > 1.0 ? 'text-accent-red' : 'text-slate-500') }}">
                                            {{ $p['opp_xg'] == 1.0 ? '-' : ($p['opp_xg'] > 1.0 ? '+' : '') . round(($p['opp_xg'] - 1) * 100) . '%' }}
                                        </span>
                                    @endif
                                </td>
                                <td class="px-4 py-2.5 text-center">
                                    <span class="{{ $p['energy'] > 1.0 ? 'text-amber-600' : ($p['energy'] < 1.0 ? 'text-accent-green' : 'text-slate-500') }}">
            ...
            <div class="bg-surface-800 rounded-lg shadow-sm overflow-hidden">
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-slate-200 text-sm">
                        <thead class="bg-surface-700/50">
                            <tr>
                                <th class="px-4 py-2.5 text-left font-semibold text-slate-300">{{ __('game.tg_line') }}</th>
                                <th class="px-4 py-2.5 text-center font-semibold text-slate-300">{{ __('game.tg_your_goals') }}</th>
                                <th class="px-4 py-2.5 text-center font-semibold text-slate-300">{{ __('game.tg_goals_conceded') }}</th>
                                <th class="px-4 py-2.5 text-left font-semibold text-slate-300 hidden md:table-cell">{{ __('game.tg_note') }}</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-slate-100">
                            @foreach($guideDefensiveLines as $d)
                            <tr>
                                <td class="px-4 py-2.5 font-semibold text-white">{{ $d['label'] }}</td>
                                <td class="px-4 py-2.5 text-center">
                                    <span class="{{ $d['own_xg'] > 1.0 ? 'text-accent-green' : ($d['own_xg'] < 1.0 ? 'text-accent-red' : 'text-slate-500') }}">
                                        {{ $d['own_xg'] == 1.0 ? '-' : ($d['own_xg'] > 1.0 ? '+' : '') . round(($d['own_xg'] - 1) * 100) . '%' }}
                                    </span>
                                </td>
                                <td class="px-4 py-2.5 text-center">
                                    <span class="{{ $d['opp_xg'] < 1.0 ? 'text-accent-green' : ($d['opp_xg'] > 1.0 ? 'text-accent-red' : 'text-slate-500') }}">
                                        {{ $d['opp_xg'] == 1.0 ? '-' : ($d['opp_xg'] > 1.0 ? '+' : '') . round(($d['opp_xg'] - 1) * 100) . '%' }}
                                    </span>
                                </td>
                                <td class="px-4 py-2.5 hidden md:table-cell text-xs text-slate-500">
                                    @if($d['threshold'] > 0)
        ...
        <section>
            ...
            <p class="text-sm text-slate-500 mb-3">{{ __('game.tg_interactions_intro') }}</p>
            <div class="space-y-3">
        ...
        <section>
            <div class="bg-surface-700/50 border border-white/10 rounded-lg p-4">
                <h3 class="text-sm font-semibold text-slate-300 mb-2">{{ __('game.tg_legend') }}</h3>
                <div class="grid grid-cols-1 md:grid-cols-3 gap-3 text-xs">
                    <div class="flex items-center gap-2">
                        <span class="text-accent-green font-semibold">+5%</span>
                        <span class="text-slate-500">{{ __('game.tg_legend_positive') }}</span>
                    </div>
                    <div class="flex items-center gap-2">
                        <span class="text-accent-red font-semibold">-5%</span>
                        <span class="text-slate-500">{{ __('game.tg_legend_negative') }}</span>
                    </div>
                    <div class="flex items-center gap-2">
                        <span class="text-amber-600 font-semibold">+10%</span>
                        <span class="text-slate-500">{{ __('game.tg_legend_energy') }}</span>
                    </div>
                </div>
            </div>
        </section>
