<span class="text-xs font-mono text-text-secondary w-8 text-right shrink-0"
...
<div class="flex-1 min-w-0">
    <span class="font-semibold text-sm text-text-primary" x-text="event.type === 'substitution' ? event.playerInName : event.playerName"></span>
    <template x-if="event.type === 'goal'">
        <span class="text-xs text-text-muted ml-1">{{ __('game.live_goal') }}</span>
    </template>
    ...
    <template x-if="event.type === 'yellow_card'">
        <span class="text-xs text-text-muted ml-1">{{ __('game.live_yellow_card') }}</span>
    </template>
    ...
    <template x-if="event.type === 'substitution'">
        <div class="text-xs text-text-secondary" x-text="'&#8617; ' + event.playerName"></div>
    </template>
    <template x-if="event.assistPlayerName">
        <div class="text-xs text-text-secondary" x-text="'{{ __('game.live_assist') }} ' + event.assistPlayerName"></div>
    </template>
</div>
