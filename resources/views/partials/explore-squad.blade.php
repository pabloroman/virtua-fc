...
<div class="flex items-center gap-4 mb-5 pb-4 border-b border-white/5">
    ...
    <div class="min-w-0">
        <h3 class="text-lg font-bold text-white truncate">{{ $team->name }}</h3>
    </div>
</div>
    ...
    <table class="w-full text-sm">
        ...
        <tbody>
            ...
            <tr class="border-b border-slate-50 hover:bg-surface-700/50/50">
                ...
                {{-- Age --}}
                <td class="py-0 pr-3 hidden md:table-cell text-slate-400">{{ $gp->age($game->current_date) }}</td>
                {{-- Market value --}}
                <td class="py-0 pr-3 hidden md:table-cell text-slate-400 font-medium">{{ \App\Support\Money::format($gp->market_value_cents) }}</td>
                {{-- Contract --}}
