    ...
    <div class="px-5 py-4 border-t border-border-default flex flex-wrap items-center gap-2">
        ...
        @if($canRenew || $renewalNegotiation?->isCountered())
            <x-action-button color="green" type="button"
                onclick="window.dispatchEvent(new CustomEvent('open-negotiation', { detail: { playerName: {{ \Illuminate\Support\Js::from($gamePlayer->name) }}, negotiateUrl: {{ \Illuminate\Support\Js::from(route('game.negotiate.renewal', [$game->id, $gamePlayer->id])) }} } }))">
