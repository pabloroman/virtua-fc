                        ...
                        <table class="w-full text-sm">
                            ...
                            <tbody id="squad-body">
                                @foreach([
                                    ...
                                    ['name' => __('squad.midfielders'), 'players' => $midfielders, 'border' => 'border-l-emerald-600', 'groupKey' => 'mf'],
                                    ['name' => __('squad.forwards'), 'players' => $forwards, 'border' => 'border-l-red-600', 'groupKey' => 'fw'],
                                ] as $group)
                                    @if($group['players']->isNotEmpty())
                                        <tr class="group-label">
                                            <td colspan="9" class="pt-5 pb-1.5 pl-4">
                                                <span class="text-[11px] font-semibold text-slate-400 uppercase tracking-wider">{{ $group['name'] }}</span>
                                            </td>
                                        </tr>
                                        @foreach($group['players'] as $gamePlayer)
                                            ...
                                            <tr class="border-l-4 {{ $group['border'] }} border-b border-slate-100 hover:bg-slate-50 player-row @if($isUnavailable) opacity-60 @endif"
                                                ...
                                                <td class="py-2.5 text-center">
                                                    <div class="flex items-center justify-center gap-1.5">
                                                        @if($gamePlayer->nationality_flag)
                                                            <img src="/flags/{{ $gamePlayer->nationality_flag['code'] }}.svg" class="w-4 h-3 rounded shadow-sm" title="{{ $gamePlayer->nationality_flag['name'] }}">
                                                        @endif
                                                        <span>{{ $gamePlayer->player->age }}</span>
                                                    </div>
                                                </td>

                                                {{-- Value + Wage + Contract --}}
                                                <td class="py-2.5 pr-3 text-right">
                                                    <div class="font-medium text-slate-900">{{ $gamePlayer->formatted_market_value }}</div>
                                                    <div class="text-xs text-slate-400">
                                                        {{ $gamePlayer->formatted_wage }}{{ __('squad.per_year') }}
                                                        @if($gamePlayer->contract_until)
                                                            &middot;
                                                            @if($gamePlayer->isContractExpiring())
                                                                <span class="text-red-500 font-medium">{{ $gamePlayer->contract_expiry_year }}</span>
                                                            @else
                                                ...
                                                {{-- Technical with bar --}}
                                                <td class="py-2.5 text-center border-l border-slate-100">
                                                ...
                                                {{-- Fitness --}}
                                                <td class="py-2.5 text-center">
                                                    <span class="text-sm @if($gamePlayer->fitness >= 90) text-green-600 font-medium @elseif($gamePlayer->fitness >= 80) text-lime-600 @elseif($gamePlayer->fitness < 50) text-red-500 font-medium @elseif($gamePlayer->fitness < 70) text-yellow-600 @endif">
                                                        {{ $gamePlayer->fitness }}
                                                    </span>
                                                </td>
                                                ...
                                                {{-- Morale --}}
                                                <td class="py-2.5 text-center">
                                                    <span class="text-sm @if($gamePlayer->morale >= 85) text-green-600 font-medium @elseif($gamePlayer->morale >= 75) text-lime-600 @elseif($gamePlayer->morale < 50) text-red-500 font-medium @elseif($gamePlayer->morale < 65) text-yellow-600 @endif">
                                                        {{ $gamePlayer->morale }}
                                                    </span>
                                                </td>
                                                ...
                                                <td class="py-2.5 text-center">
                                                    <span class="inline-flex items-center justify-center w-8 h-8 rounded-full font-bold text-sm
                                                        @if($gamePlayer->overall_score >= 80) bg-green-100 text-green-700
                                                        @elseif($gamePlayer->overall_score >= 70) bg-lime-100 text-lime-700
                                                        @elseif($gamePlayer->overall_score >= 60) bg-amber-100 text-amber-700
                                                ...
                                                <td class="py-2.5 text-right pr-2">
                                                    @if($gamePlayer->isTransferListed())
                                                        <div x-data="{ open: false }" class="relative inline-block">
                                                            ...
                                                            <div x-show="open" @click.away="open = false" x-transition class="absolute right-0 mt-1 w-36 bg-white rounded-lg shadow-lg border border-slate-200 py-1 z-10">
                                                                <form method="post" action="{{ route('game.transfers.unlist', [$game->id, $gamePlayer->id]) }}">
                                                                    @csrf
                                                                    <button type="submit" class="w-full text-left px-3 py-1.5 text-xs text-red-600 hover:bg-red-50">{{ __('squad.unlist_from_sale') }}</button>
                                                                </form>
                                                                @if($isTransferWindow)
                                                                <form method="post" action="{{ route('game.loans.out', [$game->id, $gamePlayer->id]) }}">
                                                                    @csrf
                                                                    <button type="submit" class="w-full text-left px-3 py-1.5 text-xs text-amber-600 hover:bg-amber-50">{{ __('squad.loan_out') }}</button>
                                                                </form>
                                                                @endif
                                                            </div>
                                                        </div>
                                                        ...
                                                        <div x-data="{ open: false }" class="relative inline-block">
                                                            ...
                                                            <div x-show="open" @click.away="open = false" x-transition class="absolute right-0 mt-1 w-36 bg-white rounded-lg shadow-lg border border-slate-200 py-1 z-10">
                                                                <form method="post" action="{{ route('game.transfers.list', [$game->id, $gamePlayer->id]) }}">
                                                                    @csrf
                                                                    <button type="submit" class="w-full text-left px-3 py-1.5 text-xs text-sky-600 hover:bg-sky-50">{{ __('squad.list_for_sale') }}</button>
                                                                </form>
                                                                @if($isTransferWindow)
                                                                <form method="post" action="{{ route('game.loans.out', [$game->id, $gamePlayer->id]) }}">
                                                                    @csrf
                    ...
                    <div class="pt-5 border-t mt-2">
                        <div class="flex flex-wrap items-center gap-6 text-sm text-slate-600">
                            <div>
                                ...
                                <span class="text-slate-400 ml-0.5">{{ __('app.players') }}</span>
                            </div>
                            <div class="flex items-center gap-1.5">
                                <x-position-badge abbreviation="GK" size="sm" /><span class="font-medium">{{ $goalkeepers->count() }}</span>
                                <x-position-badge abbreviation="DF" size="sm" /><span class="font-medium">{{ $defenders->count() }}</span>
                                <x-position-badge abbreviation="MF" size="sm" /><span class="font-medium">{{ $midfielders->count() }}</span>
                                <x-position-badge abbreviation="FW" size="sm" /><span class="font-medium">{{ $forwards->count() }}</span>
                            </div>
                            ...
                            <div class="border-l pl-6 flex items-center gap-1.5">
                                <span class="text-slate-400">{{ __('squad.avg_fitness') }}:</span>
                                <div class="w-12 h-1.5 bg-slate-200 rounded-full overflow-hidden">
                                    <div class="h-full rounded-full @if($avgFitness >= 85) bg-green-500 @elseif($avgFitness >= 70) bg-lime-500 @else bg-amber-500 @endif" style="width: {{ min(100, $avgFitness) }}%"></div>
                                </div>
                            ...
                            <div class="flex items-center gap-1.5">
                                ...
                                <div class="w-12 h-1.5 bg-slate-200 rounded-full overflow-hidden">
                                    <div class="h-full rounded-full @if($avgMorale >= 80) bg-green-500 @elseif($avgMorale >= 65) bg-lime-500 @else bg-amber-500 @endif" style="width: {{ min(100, $avgMorale) }}%"></div>
                                </div>
