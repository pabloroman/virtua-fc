                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <button type="button"
                                    ...
                                    <div class="flex-1 min-w-0">
                                        <h3 class="font-bold text-lg" :class="mode === 'career' ? 'text-red-900' : 'text-text-body'">
                                            {{ __('game.mode_career') }}
                                        </h3>
                                ...
                                <button type="button"
                                    ...
                                    <div class="flex-1 min-w-0">
                                        <h3 class="font-bold text-lg" :class="mode === 'tournament' ? 'text-amber-900' : 'text-text-body'">
                                        ...
                                        <div x-show="openTab === '{{ $competition->id }}'">
                                            <div class="grid lg:grid-cols-4 md:grid-cols-2 gap-2 mt-4">
                                                @foreach($competition->teams as $team)
                                                    <label class="border text-text-body has-checked:ring-sky-200 has-checked:text-accent-blue has-checked:bg-accent-blue/10 grid grid-cols-[40px_1fr_auto] items-center gap-4 rounded-lg p-4 ring-1 ring-transparent hover:bg-accent-blue/10">
                            ...
                            <div x-show="mode === 'tournament'" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" class="space-y-6">
                                ...
                                <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
                                    @foreach($wcFeaturedTeams as $team)
                                        <label class="border text-text-body has-checked:ring-amber-300 has-checked:text-amber-900 has-checked:bg-amber-100 flex flex-col items-center gap-2 rounded-xl p-4 md:p-5 ring-1 ring-transparent hover:bg-accent-gold/10 cursor-pointer transition-all">
                                ...
                                <div class="relative">
                                    ...
                                    <div class="relative flex justify-center">
                                        <span class="bg-surface-800 px-3 text-xs text-text-secondary uppercase tracking-wide">{{ __('app.all_teams') }}</span>
                                    </div>
                                </div>
                                ...
                                <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-2">
                                    @foreach($wcTeams as $team)
                                        <label class="border text-text-body has-checked:ring-amber-300 has-checked:text-amber-900 has-checked:bg-amber-100 flex items-center gap-2.5 rounded-lg p-3 ring-1 ring-transparent hover:bg-accent-gold/10 cursor-pointer transition-all">
