                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <button type="button"
                                        ...
                                        :class="mode === 'career'
                                            ...
                                            : 'border-border-strong hover:border-border-strong hover:bg-surface-700/50'"
                                    ...
                                    <div class="shrink-0 w-12 h-12 md:w-14 md:h-14 rounded-xl flex items-center justify-center"
                                         :class="mode === 'career' ? 'bg-red-600' : 'bg-surface-600'">
                                    ...
                                    <div class="flex-1 min-w-0">
                                        <h3 class="font-bold text-lg" :class="mode === 'career' ? 'text-accent-red' : 'text-text-body'">
                                            {{ __('game.mode_career') }}
                                        </h3>
                                        <p class="text-sm mt-0.5 truncate" :class="mode === 'career' ? 'text-accent-red' : 'text-text-muted'">
                                ...
                                <button type="button"
                                    ...
                                    <div class="flex-1 min-w-0">
                                        <h3 class="font-bold text-lg" :class="mode === 'tournament' ? 'text-accent-gold' : 'text-text-body'">
                                            {{ __('game.mode_tournament') }}
                                        </h3>
                                        <p class="text-sm mt-0.5 truncate" :class="mode === 'tournament' ? 'text-accent-gold' : 'text-text-muted'">
                                        ...
                                        <div x-show="openTab === '{{ $competition->id }}'">
                                            <div class="grid lg:grid-cols-4 md:grid-cols-2 gap-2 mt-4">
                                                @foreach($competition->teams as $team)
                                                    <label class="border text-text-body has-checked:ring-accent-blue/20 has-checked:text-accent-blue has-checked:bg-accent-blue/10 grid grid-cols-[40px_1fr_auto] items-center gap-4 rounded-lg p-4 ring-1 ring-transparent hover:bg-accent-blue/10">
                                                        <x-team-crest :team="$team" class="w-10 h-10" />
                                                        <span class="text-[20px]">{{ $team->name }}</span>
                                                        <input x-bind:required="mode === 'career'" x-bind:disabled="mode !== 'career'" type="radio" name="team_id" value="{{ $team->id }}" class="hidden appearance-none rounded-full border-[5px] border-white bg-surface-800 bg-clip-padding outline-hidden ring-1 ring-gray-950/10 checked:border-accent-blue checked:ring-accent-blue focus:outline-hidden">
                                                    </label>
                                                @endforeach
                                            </div>
                                        </div>
                            ...
                            <div x-show="mode === 'tournament'" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" class="space-y-6">
                                ...
                                <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
                                    @foreach($wcFeaturedTeams as $team)
                                        <label class="border text-text-body has-checked:ring-accent-gold/20 has-checked:text-accent-gold has-checked:bg-accent-gold/10 flex flex-col items-center gap-2 rounded-xl p-4 md:p-5 ring-1 ring-transparent hover:bg-accent-gold/10 cursor-pointer transition-all">
                                ...
                                <div class="relative">
                                    ...
                                    <div class="relative flex justify-center">
                                        <span class="bg-surface-900 px-3 text-xs text-text-secondary uppercase tracking-wide">{{ __('app.all_teams') }}</span>
                                    </div>
                                </div>
                                ...
                                <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-2">
                                    @foreach($wcTeams as $team)
                                        <label class="border text-text-body has-checked:ring-accent-gold/20 has-checked:text-accent-gold has-checked:bg-accent-gold/10 flex items-center gap-2.5 rounded-lg p-3 ring-1 ring-transparent hover:bg-accent-gold/10 cursor-pointer transition-all">
