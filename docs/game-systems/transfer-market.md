...
The **buy side** moves too, so the user feels the same pressure from the other direction: an AI club opens lower for one of their final-year players and won't be pushed up to full market value for him in counter-negotiation. Two deliberate differences from the sell side. It keys on contract length only — an AI club has no `Game` of its own, so there is no reputation step to measure the player's appetite for that specific move against. And it bites in the final year only, rather than grading across the leverage curve: a buyer's discount is a blunt multiplier on the whole fee, and a club cannot wait out a two-year contract without losing a season of the player. The seller's floor can decay gradually because it is only a floor — the computed asking price usually sits above it.
