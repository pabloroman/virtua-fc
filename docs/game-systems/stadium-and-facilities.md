...
A first slice of the naming-rights pillar, shipped ahead of the broader Phase 4/5 commercial rework. It is the first concrete realisation of the **commercial revenue lever**: recurring sponsor income that feeds `projected_total_revenue` and strengthens the club budget. The lever is **proactive** — it lives on a dedicated **Commercial page** (`game.club.commercial`, in the Club hub), not in a stream of unsolicited offers — so managers can see it and act on it on their own schedule. Two levers, both gated to the **pre-season identity window** (pre-season through the first league matchday, mirroring how season-ticket pricing locks):

- **Cosmetic rename** — change the stadium name freely, once per season. No fan-support effect; pure identity. Lives on the stadium page; blocked while a naming-rights deal owns the name.
- **Naming-rights sponsorship** — the manager **seeks sponsors** on demand (`NamingRightsService::seekSponsors`): each search charges a reputation-scaled **agency fee** (drawn from the transfer budget, like a cash stadium purchase) and a **cooldown** (`commercial.naming_rights.search_fee` / `search_cooldown_days`), then tops the offer board up to `max_pending_offers` with fresh, reputation-weighted offers (1–5 season terms). The headline value and the candidate brands are both tier-gated: the annual figure is drawn from the club's reputation band (`commercial.naming_rights.annual_value`, ≈€50K/yr local up to ≈€30M/yr elite, calibrated to real naming-rights deals), and the sponsor is drawn from a real-brand pool gated on two axes (`commercial.naming_rights.sponsors`): **reach** must bid for the club's tier (`sponsor_reach_by_tier`) and, for non-global brands, **country** must match the club's league — a national/regional brand only operates in its home market, while global brands (Emirates, Coca-Cola) name grounds anywhere. So a Spanish third-tier ground gets a Spanish regional backer, not a global airline or a foreign supermarket. Each playable league (ES/EN/DE/FR/IT) carries its own national + regional brands; the global pool is shared. Accepting one pays a **fixed recurring annual fee** and inflicts a **one-time fan-loyalty shock** at signing, scaled by `base_loyalty` (cult clubs pay most). The shock respects the existing `base_loyalty − 15` loyalty floor, so it's sharp but recovers through results. The fee + cooldown are the friction that keeps recurring income from becoming free money on tap.

Each offer card surfaces the **annual fee** and **contract length** — a single fixed figure the manager weighs against the fan-support cost.

**The trade-off:** the income is a known, fixed cheque, but signing costs a one-time loyalty shock that lowers the demand-curve fill rate → lower attendance → smaller gate revenue. The naming fee itself does not move with attendance.
...
- *Settled* = the same fixed `annual_value`, by `SeasonSettlementProcessor`. Projection and settlement agree exactly — no attendance variance.

**Game-scoped name:** the in-game name lives on `game_stadiums.stadium_name` (set by rename or active deal), resolved by `GameStadiumResolver` (which resolves both the game-scoped name and capacity) and surfaced through `GameMatch::venueName()`. `Team.stadium_name` remains the fallback. `GenerateNamingRightsOffersProcessor` (priority 105, before budget projection) rolls deals over each pre-season — expiring ended deals, handing the name back, minting an incumbent **renewal** offer (see below), and dropping a one-off "commercial window open" notification that deep-links to the Commercial page. It does **not** mint fresh sponsor offers; the manager seeks those.

**Pre-existing real-world deals + renewal:** clubs that begin the game already wearing a sponsor's name (Spotify Camp Nou, Emirates Stadium, …) start with a **pre-existing active deal** rather than a free "historic" name — so they earn the fee and can't double-sell. `SeedInitialNamingDealProcessor` (priority 104, before rollover 105 and budget projection 107) materialises it once at game setup from `commercial.preexisting_naming_deals`, an overlay keyed by `transfermarkt_id` that supplies the sponsor brand and the **clean traditional name** to revert to. The sponsored display name comes from the untouched seed `Team.stadium_name`, so AI opponents keep their real branding via the resolver fallback. The fee is tier-derived (no per-club value authored) and seeding applies **no** loyalty shock (the fans already live with the name). The deal runs a short term (`preexisting_deal_seasons`, 1 season), so each pre-season it lapses (the ground reverts to the traditional name) and the incumbent offers a **free renewal**: `rolloverForNewSeason` mints a pending `is_renewal` offer for the same sponsor, re-priced to the club's **current** reputation tier and granting `renewal_seasons` (1) more. Accepting a renewal re-brands the ground with **no** loyalty shock (the name is unchanged); declining leaves it on the traditional name, with the paid seek flow still available to shop for a different sponsor (which does cost loyalty). Renewal applies to *any* expiring deal — seeded or agency-signed.
...
| `app/Modules/Stadium/Services/GameStadiumResolver.php` | Resolves the game-scoped stadium name (override → `Team.stadium_name` fallback) and capacity |
...
| `app/Modules/Season/Processors/GenerateNamingRightsOffersProcessor.php` | Pre-season ended-deal expiry, incumbent renewal-offer minting, + once-per-pre-season "window open" nudge (priority 105). Does not mint fresh sponsor offers |
| `app/Modules/Season/Processors/SeedInitialNamingDealProcessor.php` | Materialises a club's pre-existing real-world naming deal once at setup (priority 104, before rollover + budget projection) |
| `app/Models/GameStadiumNamingDeal.php` | Offer/active/history rows for naming-rights deals; `is_renewal` flags incumbent renewals (no loyalty shock on accept) |
| `config/commercial.php` | Commercial-lever config (split out of `finances.php`): naming-rights tiered `annual_value`, `search_fee`, `sponsor_reach_by_tier`, the reach+country-tagged `sponsors` brand pool, and the `preexisting_naming_deals` overlay (real-world deals keyed by `transfermarkt_id`) |
